...
int  HSF_API hfnet_get_dhcp_ip (int * ip, int *mask, int *gw);/*Undefined*/

extern int net_ping_test(unsigned int dst, int seq, int ping_time);
extern unsigned int nstr2ip(char *hn);

#define hfnet_ping(_ipadr)		net_ping_test(nstr2ip(_ipadr), 1, 1*1000)//1s
...
int HSF_API hfnet_recvfrom(int s, void *mem, size_t len, int flags, struct sockaddr *from, socklen_t *fromlen,char * mac);
