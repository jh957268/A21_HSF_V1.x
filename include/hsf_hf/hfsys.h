...
#define HFSYS_RESET_REASON_USER_TEST		(0x00000100)
...
void HSF_API hfsys_softreset(void);/*?1*/
 ...
 */
uint32_t HSF_API hfsys_get_memory(void);/*?2*/
/*
  *
  */
...
int HSF_API hfsys_nvm_read(uint32_t nvm_addr, char* buf, uint32_t length);/*?3*/

int HSF_API hfsys_nvm_write(uint32_t nvm_addr, char* buf, uint32_t length);/*?4*/

void u_delay(uint32_t n);
