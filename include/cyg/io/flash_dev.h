...
struct cyg_flash_dev_funs {
  ...
  int     (*flash_block_unlock) (struct cyg_flash_dev *dev, 
                                 const cyg_flashaddr_t block_base);
#endif
  int (*flash_erase_sector)(struct cyg_flash_dev *dev, 
                                cyg_flashaddr_t sector_base);
...
//menghui add flash_erase_sector
// Macros for instantiating the above structures.
#ifdef CYGHWR_IO_FLASH_BLOCK_LOCKING
# define CYG_FLASH_FUNS(_funs_, _init_, _query_ , _erase_, _prog_ , _read_, _lock_, _unlock_,_erase_se_) \
